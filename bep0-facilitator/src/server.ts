// bep0 Facilitator Service - Matches x402.org API
...
const BEP0_RELAYER_ADDRESS = process.env.BEP0_RELAYER_ADDRESS!;

if (!RELAYER_PRIVATE_KEY || !BEP0_RELAYER_ADDRESS) {
  console.error('❌ Missing required env vars: RELAYER_PRIVATE_KEY, BEP0_RELAYER_ADDRESS');
  process.exit(1);
}
...
app.post('/settle', async (req, res) => {
  ...
  try {
    ...
    console.log(`   Relayer: ${BEP0_RELAYER_ADDRESS}`);
    console.log('');
...
app.listen(PORT, () => {
  console.log('🔥 bep0 Facilitator Service');
  console.log(`📡 Listening on http://localhost:${PORT}`);
  console.log(`🔑 Relayer: ${relayerWallet.address}`);
  console.log(`📝 Contract: ${BEP0_RELAYER_ADDRESS}`);
  console.log('');
